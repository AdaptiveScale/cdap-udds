...
@Name(FormatPreservingEncrypt.DIRECTIVE_NAME)
@Description("Format Preserving Encrypt")
public class FormatPreservingEncrypt implements Directive {

    public static final String DIRECTIVE_NAME = "fp-encrypt";
    ...
    public List<Row> execute(List<Row> rows, ExecutorContext executorContext) throws DirectiveExecutionException, ErrorRowException, ReportErrorAndProceed {
        for (Row row : rows) {
            ...
            if (idx != -1) {
                ...
                String cipherText = formatPreservingEncryption.encrypt(value, FormatPreservingUtil.TWEAK);
